...
const qseowUpdateSheetThumbnails = async options => {
  try {
    ...
    if (sheetListObj.qAppObjectList.qItems.length > 0) {
      ...
      logger.info(`UPDATE: Number of sheets in app: ${sheetListObj.qAppObjectList.qItems.length}`);
      ...
      for (const sheet of sheetListObj.qAppObjectList.qItems) {
        ...
        let sheetObj = await app.getObject(sheet.qInfo.qId);
        let sheetProperties = await sheetObj.getProperties();
    ...
    if ((await session.close()) == true) {
      ...
      logger.error(`Error closing session for QSEoW app ${options.appid} on host ${options.host}`);
    }
  } catch (err) {
