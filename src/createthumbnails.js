...
const qseowCreateThumbnails = (options) =>
    new Promise(async (resolve, reject) => {
        try {
            ...
            logger.info(
                'Starting creation of thumbnails for Qlik Sense Enterprise on Windows (QSEoW)'
            );
            ...
            if (contentLibraryExists === false) {
                logger.error(
                    `Content library '${options.contentlibrary}' does not exist - aborting`
                );
            ...
            if (sheetListObj.qAppObjectList.qItems.length > 0) {
                ...
                if (options.prefix.length > 0) {
                    appUrl = `${appUrl + options.host}/${options.prefix}/sense/app/${
                        options.appid
                    }`;
                } else {
