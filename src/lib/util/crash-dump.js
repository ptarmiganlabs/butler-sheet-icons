/**
 * Crash dump module for Butler Sheet Icons.
 *
 * Writes a crash dump file (JSON and/or plain text) when BSI encounters an
 * unrecoverable error such as an uncaught exception, unhandled promise
 * rejection, or an explicit fatal error logged via `logFatal()`.
 *
 * Key design goals:
 * - Sensitive config data (passwords, API keys, tokens, certificates) is never written
 ...
 * sensitive data depending on what the upstream error captured. Treat crash
 * dump files as potentially sensitive.
 *
 * Configuration is via environment variables (BSI is flag-driven rather than
 * config-file driven, unlike Butler SOS):
 * - `BSI_CRASH_DUMP_ENABLE`            (default: `1`) — set to `0` to disable
 ...
 * All values are string-coerced to booleans/strings via the helpers below.
 */
const ENV_ENABLE = 'BSI_CRASH_DUMP_ENABLE';
...
/**
 * Parse an env-var string as a boolean. `1`/`true`/`yes` → `true`, anything
 * else (including unset) → `false`. Case-insensitive.
 *
 * @param {string|undefined} value - Raw env-var value.
 * @returns {boolean} `true` only for the affirmative values listed above.
 */
 ...
 * Handles both POSIX and Windows path separators. Frames containing `src/`
 * keep only the relative portion; other standalone absolute path tokens are
 * replaced with `[path]`.
 *
...
function sanitizeStackTrace(stack) {
    ...
    //    do not accidentally match within already-reduced `src/...` paths.
 ...
 * Falls back to `./crash_dumps` when the configured directory is empty.
 *
 * @param {string} configuredDir - The directory path from env/config (may be relative or absolute).
 * @returns {string} Absolute path to the crash dump directory.
 */
function resolveCrashDir(configuredDir) {
    if (!configuredDir || configuredDir.trim() === '') {
        return path.resolve(process.cwd(), 'crash_dumps');
    }
 ...
 *
 * The function times out after {@link CRASH_DUMP_WRITE_TIMEOUT_MS} milliseconds
 * so it can never block process exit indefinitely on slow or hung storage. It
 * also swallows all its own errors so I/O failures can never prevent
 * process.exit().
 *
 ...
 * patterns (URLs with credentials, bearer tokens, key=value secrets). Redaction
 * cannot guarantee removal of all sensitive data from error content; treat
 * crash dump files as potentially sensitive.
 *
 ...
 *   (`uncaughtException` | `unhandledRejection` | `logFatal` | `manual`).
 *
...
export async function writeCrashDump(error, source) {
    try {
        // ----------------------------------------------------------------
        // Read crash-dump env-var configuration. All four have sane defaults
        // so the function still works when called very early in startup.
        // ----------------------------------------------------------------
        const enable =
            process.env[ENV_ENABLE] === undefined ? true : envBool(process.env[ENV_ENABLE]);
        const crashDir = process.env[ENV_DIR] || 'crash_dumps';
        const createJson =
            process.env[ENV_CREATE_JSON] === undefined
                ? true
                : envBool(process.env[ENV_CREATE_JSON]);
        const createText =
            process.env[ENV_CREATE_TEXT] === undefined
                ? true
                : envBool(process.env[ENV_CREATE_TEXT]);
        ...
        const appName = 'butler-sheet-icons';
        ...
        const crashData = {
            ...
            app: {
                name: appName,
                version: process.env.BSI_APP_VERSION || 'unknown',
            },
            runtime: {
                nodeVersion,
                platform,
        ...
        const textReport = [
            ...
            '',
            '=== APPLICATION INFO ===',
            `Butler Sheet Icons Version: ${process.env.BSI_APP_VERSION || 'unknown'}`,
        ...
        // ----------------------------------------------------------------
        // Write files. Track per-file success so the log message is only
        // emitted when the file was actually written (write errors and
        // timeout are both treated as non-success).
        // ----------------------------------------------------------------
        ...
        if (jsonWritten) {
            try {
                if (logger) {
                    logger.error(`CRASH DUMP: Written to ${jsonFilePath}`);
                } else {
        ...
        if (txtWritten) {
            try {
                if (logger) {
                    logger.error(`CRASH DUMP: Written to ${txtFilePath}`);
                } else {
