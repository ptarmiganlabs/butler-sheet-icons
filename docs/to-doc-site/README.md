...
Files in this folder are the source of truth for updates to the Butler Sheet Icons (BSI) documentation site. They are written, reviewed, and finalized here before being published to the official documentation.

## Purpose

This folder serves as a staging area for documentation that should eventually appear on the Butler Sheet Icons doc site (`butler-sheet-icons.ptarmiganlabs.com`). It is the single place where documentation is authored outside of the doc site's own repository.
...
- Files starting with `done_` have already been incorporated into the Butler Sheet Icons doc site, or their content has been verified to already exist there.
